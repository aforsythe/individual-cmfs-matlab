...
%[text] The photopic luminous efficiency function $V^{*}(\\lambda)$ gives the weight the visual system attaches to each wavelength when luminance is computed. Luminance is the quantity that flicker photometry and additive matching measure.
%[text] Luminance is not brightness. Brightness includes a contribution from the colour of the light, known as the Helmholtz-Kohlrausch effect, which $V^{*}$ deliberately leaves out.
%[text] $V^{*}(\\lambda)$ is also not the 1924 $V(\\lambda)$ function, which underestimates sensitivity at short wavelengths. It is the later function based on the cone fundamentals, and it is the y-bar row of the CIE 170-2:2015 transform from LMS to XYZ. That makes it a weighted sum of the L and M cone fundamentals:
...
%[text] The toolbox computes $V^{*}(\\lambda)$ from the Stockman and Rider (2023) cone fundamentals rather than fitting the tabulated CIE function directly. The two agree to better than 1% at the peak.
%[text] **Time:** about 8 minutes.
exampleDefaults();
%%
%[text] ## The two standard observers
%[text] For both standard observers $V^{*}(\\lambda)$ peaks near 1.0 at 555 nm.
...
%[text] $V^{*}(\\lambda)$ depends on the shape of the L cone, so observers carrying different L-opsin variants have different luminous efficiency functions. The Ser180Ala polymorphism moves the L photopigment lambda-max by 2.7 nm, and that moves $V^{*}(\\lambda)$ at long wavelengths.
...
%[text] As the lens absorbs more short-wavelength light with age, $V^{*}(\\lambda)$ decreases at short wavelengths.
%[text] At long wavelengths it increases instead, by 7 to 11 percent between ages 25 and 75. The lens absorbs almost nothing there, so normalization raises those wavelengths relative to the short ones that were reduced. The peak of $V^{*}$ also moves from 551 to 558 nm.
...
%%
%[text] ## Dichromats
%[text] For an observer missing one cone class, $V^{*}(\\lambda)$ reduces to a single term:
%[text] - A **protanope**, set with `Lod=0`, has $V^*(\\lambda) = b \\bar{M}(\\lambda)$.
%[text] - A **deuteranope**, set with `Mod=0`, has $V^*(\\lambda) = a \\bar{L}(\\lambda)$.
%[text] - A **tritanope**, set with `Sod=0`, has $V^{*}(\\lambda)$ unchanged, since the S cone does not appear in it. \
...
%[text] MacLeod-Boynton chromaticity belongs in this example because its denominator is $V^{*}(\\lambda)$. The two coordinates are $l_{MB} = aL/(aL + bM)$ and $s_{MB} = S/(aL + bM)$, using the same coefficients $(a, b)$.
...
%[text] With $V^{*}(\\lambda)$ for an observer, photometric quantities can be computed for any spectral power distribution. For a radiant flux $\\Phi_e(\\lambda)$ in watts per nanometre, the luminous flux is
...
%[text] A source of 1 W at 555 nm therefore has a luminous flux of $K_m V^{*}(555)$, which is about 683 lm. The result is approximate for two reasons. The SI definition fixes 683 lm/W at 540 THz against the 1924 $V(\\lambda)$, so using it with $V^{*}$ is a modelling choice rather than a definition, and the residual of the Stockman and Rider fit puts the result within 1% of $K_m$.
Km = 683;
wlFine = (380:0.5:780)';
...
%%
%[text] ## Key takeaways
%[text] - $V^{*}(\\lambda)$ is the y-bar row of the CIE 170-2:2015 transform from LMS to XYZ. The `Luminance` method returns it
%[text] - It always uses normalized fundamentals in energy units, whatever the observer's `OutputFormat` is set to
%[text] - Genotype and age both produce differences in $V^{*}(\\lambda)$ between individuals
%[text] - For a dichromat, $V^{*}(\\lambda)$ keeps only the term for the cone that is present. Without rescaling, a protanope peaks near 0.35 and a deuteranope near 0.69
