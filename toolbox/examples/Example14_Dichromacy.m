...
%[text] Real dichromats differ in whether the affected cones are absent altogether or are present but contain one of the other pigments. Either way only two distinct classes remain, and setting the optical density to zero models that.
...
%[text] CIE XYZ is a 3 by 3 linear transform of LMS. That matrix is fixed and invertible, and it is the same matrix for every observer. What is missing is a dimension of the observer.
...
%[text] - Setting a density selects Custom mode, which holds all three cone densities. At any field size other than the default, set the surviving densities explicitly or they keep their 10 deg values
%[text] - `XYZ` and `RGB` raise errors for a dichromat. The observer varies in two dimensions, and for `RGB` the matrix of primaries is genuinely singular. Pass a `TransformationMatrix` to override
%[text] - `Luminance` still works. The S cone contributes nothing to it, so a tritanope has the same luminous efficiency as the standard observer. See [Example 12](matlab:edit('Example12_Luminance.m'))
...
%[text] **Next:** [Example 15: Advanced Customization](matlab:edit('Example15_AdvancedCustomization.m')). Every parameter, and the `getParameters` and `setParameters` round trip.
