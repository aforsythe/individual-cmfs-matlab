...
%[text] How to get cone-fundamental data out of an observer in a form usable by other tools -- CSV, MAT, or directly into a MATLAB table or struct.
...
%[text] Record the output settings alongside the biophysics. `normalize_output` and `normalization_method` decide what the numbers mean -- normalized to a peak or absolute -- and `primaries_nm` is required to interpret the RGB columns at all, since RGB CMFs are defined relative to their primaries.
metadata = struct( ...
...
%[text] - `writetable` handles CSV (and Excel, via a `.xlsx` path and the `Sheet` option); `save` handles MAT
%[text] - CSV carries data only. Put the observer's parameters -- including `NormalizeOutput`, `NormalizationMethod` and `Primaries` -- in the MAT metadata struct, or encode them in CSV column names as the multi-observer export does, so a recipient can tell what the numbers are
