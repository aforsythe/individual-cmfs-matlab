...
%[text] `IndividualCMF.across` builds the observers. It takes the name of one parameter and a list of values, and returns one observer per value, with any further arguments applied to all of them. It is used in the same way wherever a later example sweeps a parameter.
